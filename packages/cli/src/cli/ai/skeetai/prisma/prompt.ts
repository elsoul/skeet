...
async function readPrismaSchema(modelPath: string) {
  try {
    return await readFile(modelPath, 'utf8')
  } catch (error) {
    console.log(error)
    return null
  }
...
export const prismaPrompt = async (modelPath: string) => {
  const prismaSchema = await readPrismaSchema(modelPath)
